document.addEventListener('DOMContentLoaded', function () {
    ...
    validateField('fname', value => nameRegex.test(value), 'First name should only contain letters and spaces, and must start with a letter.');
    validateField('lname', value => nameRegex.test(value), 'Last name should only contain letters and spaces, and must start with a letter.');
    validateField('number', value => numberRegex.test(value), 'Mobile number must be 11 digits.');
    ...
    document.getElementById('signupForm').addEventListener('submit', function (event) {
        ...
        if (!nameRegex.test(fname)) {
            showError('fname', 'First name should not contain numbers, and must start with a letter.');
            valid = false;
        }
        if (!nameRegex.test(lname)) {
            showError('lname', 'Last name should not contain numbers., and must start with a letter.');
            valid = false;
        }
